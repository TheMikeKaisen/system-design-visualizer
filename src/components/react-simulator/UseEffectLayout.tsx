...
import { EffectLifecyclePanel } from "./EffectLifecyclePanel";
import { DependencyArrayPanel } from "./DependencyArrayPanel";
...
export function UseEffectLayout() {
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-background overflow-hidden">
      ...
      <div className="w-[380px] sm:w-[420px] shrink-0 border-r border-border bg-card flex flex-col h-full overflow-hidden shadow-xl z-20">
        ...
        <div className="p-6 border-b border-border bg-zinc-900/40">
          <div className="text-xs font-bold tracking-widest uppercase text-emerald-500 mb-2">
            ...
            <div className="flex flex-col gap-4 mt-2">
              {step.notes.map((note, idx) => (
                <div key={idx} className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
                  <h4 className="font-semibold text-emerald-400 mb-2 text-sm">{note.title}</h4>
      ...
      <div className="flex-1 flex flex-col p-4 gap-4 overflow-hidden bg-zinc-950">
        ...
        {/* Bottom: Effect Lifecycle + Dependency Array */}
        <div className="flex-1 flex gap-4 min-h-0 w-full">
          <div className="flex-1 min-w-0">
            <EffectLifecyclePanel step={step} />
          </div>
          <div className="flex-1 min-w-0">
            <DependencyArrayPanel step={step} />
          </div>
        </div>
