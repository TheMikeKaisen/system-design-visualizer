...
function ContextCard({ node }: { node: TreeNode }) {
  ...
  // If this is a block scope, give it a slightly different internal styling
  ...
  return (
    ...
    >
      ...
      {/* Body (Only shown if active, being searched, or if it's a block (blocks are always visible if their parent is)) */}
      <AnimatePresence>
        {(isStackActive || isLookupActive || isLookupChecked || isBlock || node.children.length > 0) && (
          <motion.div 
          ...
          >
            ...
            {/* Render nested children (block scopes) */}
