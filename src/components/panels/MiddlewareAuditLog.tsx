...
export function MiddlewareAuditLog({ gatewayId }: MiddlewareAuditLogProps) {
  ...
  return (
    <div className="px-5 py-4 border-t border-black/10 dark:border-white/10 flex flex-col gap-3">
...
function AuditRow({ entry }: { entry: AuditEntry }) {
  ...
  return (
    <div className="flex items-start gap-2 p-2 bg-black/5 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-md group hover:bg-black/10 dark:hover:bg-black/40 transition-colors">
      <span className={`text-[11px] font-mono font-bold shrink-0 mt-0.5 ${verdictStyle}`}>
        {icon}
      </span>
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="flex items-center gap-1">
          <span className="text-[10px] font-mono text-zinc-700 dark:text-zinc-200">{entry.protocol}</span>
          <span className="text-[10px] text-zinc-500">·</span>
          <span className="text-[10px] text-zinc-600 dark:text-zinc-400">{entry.stepType}</span>
        </div>
        {entry.reason && (
...
function CircuitBreakerBadge({ state }: { state: string }) {
  const styles: Record<string, string> = {
    CLOSED:    "text-green-600 dark:text-green-400 bg-green-400/10 border-green-400/20 shadow-[0_0_10px_rgba(74,222,128,0.1)]",
    OPEN:      "text-red-600 dark:text-red-400 bg-red-400/10 border-red-400/20 shadow-[0_0_10px_rgba(248,113,113,0.2)] animate-pulse",
    HALF_OPEN: "text-amber-600 dark:text-amber-400 bg-amber-400/10 border-amber-400/20 shadow-[0_0_10px_rgba(251,191,36,0.1)]",
  };
  return (
