...
export default function JavaConstructorsSimulator() {
  const { 
    scenario, 
    currentStepIndex, 
    isPlaying,
  ...
  useEffect(() => {
    ...
    if (isPlaying && currentStep) {
      timer = setTimeout(() => {
        nextStep();
      }, 3500); // 3.5 second delay between steps for reading
    }
    return () => clearTimeout(timer);
  }, [isPlaying, currentStepIndex, nextStep, currentStep]);

  if (!currentStep || scenario.id !== "java-constructors") {
