...
import {
  computeUniversalScheduling,
  getUniversalProcessStateAtTime,
  DEFAULT_UNIVERSAL_PROCESSES,
  PROCESS_COLORS,
  TERM_INFO,
  type UniversalProcess,
  type UniversalSimulation,
  type UniversalProcessResult,
  type ProcessStateAtTime,
  type SchedulingAlgorithm,
} from "@/lib/os-simulator/universal-engine";
...
export default function UniversalSchedulingSimulator() {
  ...
  const processStates = useMemo(() => {
    ...
    return new Map(
      simulation.processResults.map((r) => [
        r.id,
        getUniversalProcessStateAtTime(r, currentTime),
      ])
    );
  }, [simulation, currentTime]);

  /** The process currently running on CPU */
  const runningProcess = useMemo<UniversalProcessResult | null>(() => {
    if (!simulation) return null;
    return (
      simulation.processResults.find(
        (r) => getUniversalProcessStateAtTime(r, currentTime) === "running"
      ) ?? null
    );
  }, [simulation, currentTime]);
  ...
  const cpuProgress = useMemo(() => {
    if (!runningProcess) return { elapsed: 0, remaining: 0, pct: 0 };
    
    let totalElapsed = 0;
    for (const b of runningProcess.executionBlocks) {
      if (b.endTime <= currentTime) {
        totalElapsed += (b.endTime - b.startTime);
      } else if (currentTime >= b.startTime && currentTime < b.endTime) {
        totalElapsed += (currentTime - b.startTime);
      }
    }
    
    const remaining = runningProcess.burstTime - totalElapsed;
    const pct = (totalElapsed / runningProcess.burstTime) * 100;
    return { elapsed: totalElapsed, remaining, pct };
  ...
  const readyProcesses = useMemo(
    () =>
      simulation?.processResults.filter(
        (r) => getUniversalProcessStateAtTime(r, currentTime) === "ready"
      ) ?? [],
    [simulation, currentTime]
  );
  const terminatedProcesses = useMemo(
    () =>
      simulation?.processResults.filter(
        (r) => getUniversalProcessStateAtTime(r, currentTime) === "terminated"
      ) ?? [],
    [simulation, currentTime]
  );
  ...
  const updateProcess = useCallback(
    (id: string, updates: Partial<UniversalProcess>) => {
    ...
    <div className="h-[100dvh] bg-background flex flex-col overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[380px_1fr] overflow-hidden">
        ...
        <aside className="border-r border-border/40 bg-background/40 overflow-y-auto p-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          {/* ─── SCHEDULING WARNING ─── */}
          {simulation?.warning.detected && isComplete && (
            <motion.div
            ...
            >
              <p className="text-xs font-bold text-amber-500 flex items-center gap-1.5">
                ...
                {simulation.warning.type === "convoy_effect" ? "Convoy Effect Detected" : "Starvation Detected"}
              </p>
