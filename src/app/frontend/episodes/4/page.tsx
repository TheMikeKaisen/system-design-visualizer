...
import { UseEffectLayout } from "@/components/react-simulator/UseEffectLayout";
import { useReactSimulationStore } from "@/store/useReactSimulationStore";
import { REACT_SCENARIO_4 } from "@/lib/react-simulator/react-scenario-4";

export default function ReactEpisode4Page() {
  const { scenario, currentStepIndex, setScenario, nextStep, prevStep, reset } =
    useReactSimulationStore();

  useEffect(() => {
    setScenario(REACT_SCENARIO_4);
  }, [setScenario]);
    ...
    <div className="flex flex-col h-screen bg-background font-sans overflow-hidden">
      ...
      <nav className="h-16 shrink-0 border-b border-border bg-zinc-950 flex items-center justify-between px-4 md:px-6 z-50 shadow-sm relative">
        <div className="flex items-center">
          ...
          <span className="text-sm font-semibold text-emerald-500">Episode 4</span>
