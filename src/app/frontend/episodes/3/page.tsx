...
import { UseStateBatchingLayout } from "@/components/react-simulator/UseStateBatchingLayout";
import { useReactSimulationStore } from "@/store/useReactSimulationStore";
import { REACT_SCENARIO_3 } from "@/lib/react-simulator/react-scenario-3";

export default function ReactEpisode3Page() {
  const { scenario, currentStepIndex, setScenario, nextStep, prevStep, reset } =
    useReactSimulationStore();

  useEffect(() => {
    setScenario(REACT_SCENARIO_3);
  }, [setScenario]);
    ...
    <div className="flex flex-col h-screen bg-background font-sans overflow-hidden">
      ...
      <nav className="h-16 shrink-0 border-b border-border bg-zinc-950 flex items-center justify-between px-4 md:px-6 z-50 shadow-sm relative">
        <div className="flex items-center">
          ...
          <span className="text-sm font-semibold text-amber-500">Episode 3</span>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 p-1 rounded-lg shadow-inner">
              ...
              <button
                ...
                className="flex items-center gap-1 pl-3 pr-2 py-1.5 bg-amber-600/20 hover:bg-amber-600/30 text-amber-400 rounded-md disabled:opacity-30 text-xs font-bold transition-colors shadow-sm"
              >
