...
import { UseEffectCleanupLayout } from "@/components/react-simulator/UseEffectCleanupLayout";
import { useReactSimulationStore } from "@/store/useReactSimulationStore";
import { REACT_SCENARIO_5 } from "@/lib/react-simulator/react-scenario-5";

export default function ReactEpisode5Page() {
  const { scenario, currentStepIndex, setScenario, nextStep, prevStep, reset } =
    useReactSimulationStore();

  useEffect(() => {
    setScenario(REACT_SCENARIO_5);
  }, [setScenario]);
    ...
    <div className="flex flex-col h-screen bg-background font-sans overflow-hidden">
      ...
      <nav className="h-16 shrink-0 border-b border-border bg-zinc-950 flex items-center justify-between px-4 md:px-6 z-50 shadow-sm relative">
        <div className="flex items-center">
          ...
          <span className="text-sm font-semibold text-rose-500">Episode 5</span>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 p-1 rounded-lg shadow-inner">
              ...
              <button
                ...
                className="flex items-center gap-1 pl-3 pr-2 py-1.5 bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 rounded-md disabled:opacity-30 text-xs font-bold transition-colors shadow-sm"
              >
