...
import { PropsFlowLayout } from "@/components/react-simulator/PropsFlowLayout";
import { useReactSimulationStore } from "@/store/useReactSimulationStore";
import { REACT_SCENARIO_2 } from "@/lib/react-simulator/react-scenario-2";

export default function ReactEpisode2Page() {
  const { scenario, currentStepIndex, setScenario, nextStep, prevStep, reset } =
    useReactSimulationStore();

  useEffect(() => {
    setScenario(REACT_SCENARIO_2);
  }, [setScenario]);
    ...
    <div className="flex flex-col h-screen bg-background font-sans overflow-hidden">
      ...
      <nav className="h-16 shrink-0 border-b border-border bg-zinc-950 flex items-center justify-between px-4 md:px-6 z-50 shadow-sm relative">
        <div className="flex items-center">
          ...
          <span className="text-sm font-semibold text-blue-500">Episode 2</span>
          <span className="mx-2 text-muted-foreground">•</span>
          <span className="text-sm font-medium text-foreground truncate">Props, State & One-Way Data Flow</span>
        </div>
