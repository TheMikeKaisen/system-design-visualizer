...
import { ReactSimulatorLayout } from "@/components/react-simulator/ReactSimulatorLayout";
import { useReactSimulationStore } from "@/store/useReactSimulationStore";
import { REACT_SCENARIO_1 } from "@/lib/react-simulator/react-scenario-1";

export default function ReactEpisode1Page() {
  const { scenario, currentStepIndex, setScenario, nextStep, prevStep, reset } = useReactSimulationStore();

  useEffect(() => {
    // Load the scenario when the page mounts
    setScenario(REACT_SCENARIO_1);
  }, [setScenario]);

  return (
    <div className="flex flex-col h-screen bg-background font-sans overflow-hidden">
      {/* Navbar specific to the episode */}
      <nav className="h-16 shrink-0 border-b border-border bg-zinc-950 flex items-center justify-between px-4 md:px-6 z-50 shadow-sm relative">
        <div className="flex items-center">
          ...
          <span className="text-sm font-semibold text-blue-500">Episode 1</span>
          <span className="mx-2 text-muted-foreground">•</span>
          <span className="text-sm font-medium text-foreground truncate">JSX & Virtual DOM</span>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 p-1 rounded-lg shadow-inner">
              ...
              <button 
                ...
                className="flex items-center gap-1 pl-3 pr-2 py-1.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded-md disabled:opacity-30 text-xs font-bold transition-colors shadow-sm"
              >
      ...
      {/* Simulator Engine Layout */}
      <ReactSimulatorLayout />
    </div>
  );
}
