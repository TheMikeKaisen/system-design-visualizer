...
import { BLOCK_SCOPE_SCENARIOS } from "@/lib/js-simulator/block-scope-scenarios";

export default function JSBlockScopeSimulator() {
  const { 
    scenario, 
    currentStepIndex, 
    isPlaying,
  ...
  // Initialize with the first block scope scenario on mount
  useEffect(() => {
    if (!BLOCK_SCOPE_SCENARIOS.find(s => s.id === scenario.id)) {
      setScenario(BLOCK_SCOPE_SCENARIOS[0]);
    }
