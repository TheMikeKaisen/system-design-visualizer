...
import { LET_CONST_SCENARIOS } from "@/lib/js-simulator/let-const-scenarios";

export default function JSLetConstSimulator() {
  const { 
    scenario, 
    currentStepIndex, 
    isPlaying,
  ...
  // Initialize with the first let-const scenario on mount
  useEffect(() => {
    // Only set if we aren't already on a let-const scenario
    if (!LET_CONST_SCENARIOS.find(s => s.id === scenario.id)) {
      setScenario(LET_CONST_SCENARIOS[0]);
    }
          ...
          {scenario.steps[currentStepIndex]?.visualEffect && (
            ...
            >
              ...
              >
                ...
                {scenario.steps[currentStepIndex].visualEffect!.reason ? (
                  scenario.steps[currentStepIndex].visualEffect!.reason
                ) : (
                  <>
